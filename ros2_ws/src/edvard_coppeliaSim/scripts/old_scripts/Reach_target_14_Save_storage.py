'''
Currently the whole relative path of the deleted episode is shown, can easily change that 
by using this instead: print(f"Deleting directory: {os.path.basename(episode_dirs[0])}")
'''
...
RUN_NAME = 'Test_Save_storage'
LOAD_PREVIOUS_RUN = True
PREVIOUS_RUN_PATH = '/home/h/Edvard2/ros2_ws/src/coppeliasim_launch/scripts/Reach_target_Recordings/Run_01/best_episode/best_model.pth'
...
class RunManager:
    ...
    def compare_and_update_best_episode(self, current_reward, current_episode_dir):
        if current_reward > self.best_reward:
            self.best_reward = current_reward
            best_episode_dir = self.run_dir / 'best_episode'
            if best_episode_dir.exists():
                shutil.rmtree(best_episode_dir)
            shutil.copytree(current_episode_dir, best_episode_dir)
            print(f"New best episode found. Updated best episode to: {os.path.basename(current_episode_dir)}") 
            return True
        return False
 
    def make_video(self, directory, image_type):
        assert image_type in ['RGB', 'depth'], "Invalid image_type argument. Choose either 'RGB' or 'depth'."

        if image_type == 'RGB':
            images = np.load(f'{directory}/images.npy')
        else:
            images = np.load(f'{directory}/depth_images.npy')
        ...
        clip.write_videofile(f'{output_dir}/{image_type}_video.mp4')
...
for e in range(EPISODES):
    ...
    for i in range(EPISODE_LENGTH):
        ...
        states.append(state)
        images.append(image)
        depth_images.append(image_depth)
    ...
    if run_manager.compare_and_update_best_episode(episode_reward, episode_dir):
        torch.save(agent.policy.state_dict(), os.path.join(best_episode_dir, 'best_model.pth'))
        best_episodes.append(e)
...
run_manager.make_video(BEST_EPISODE_DIR, 'RGB')
run_manager.make_video(BEST_EPISODE_DIR, 'depth')
print('Done!')
env.shutdown()
