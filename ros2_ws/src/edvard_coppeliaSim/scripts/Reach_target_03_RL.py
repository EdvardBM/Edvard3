...
class Policy(nn.Module):
    ...
    def forward(self, state):
        #Debug Print: print(f'State shape in forward: {state.shape}') 
        mean = self.actor(state)
        mean = self.actor(state).unsqueeze(0)
        std = self.log_std.exp().expand_as(mean)
        #Debug Print: print(f'Mean shape: {mean.shape}, Std shape: {std.shape}') 
...
class Agent(object):
    ...
    def act(self, state):
        #Debug Print: print(f'State shape in act: {state.shape}') 
        state = torch.tensor(state, dtype=torch.float32)
        with torch.no_grad():
            dist, value = self.policy(state)
        action = dist.sample()
        #Debug Print: print(f'Action shape in act: {action.shape}')  
        log_prob = dist.log_prob(action)
        return action.numpy().squeeze(), value.item(), log_prob.item()

    def learn(self, states, actions, log_probs_old, rewards, masks):
...
for e in range(EPISODES):
    ...
    state = env.reset()
    #Debug Print: print(f'State shape in main loop: {state.shape}')  
    states = []
    actions = []
    log_probs_old = []
    values = []
    ...
    for i in range(EPISODE_LENGTH):
        ...
        state = next_state
    agent.learn(states, actions, log_probs_old, rewards, masks)
print('Done!')
env.shutdown()
